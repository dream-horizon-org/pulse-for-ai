...
This is a kill switch, not a feature toggle. `true` does not enable screen tracking — you still need `useNavigationTracking`. Set to `false` to fully disable navigation instrumentation.

## networkHeaders

Capture custom HTTP headers alongside each network span. Only listed header names are captured — all others are ignored.
...
Use `__DEV__` to guard log level:
